...
# Save output
write.csv(specieslist, file.path(path_outputs, "12SRefDB_Step3Output.csv"), row.names = FALSE)
```


## *Step R4: Scrape the target locus out of mitogenomes identified from Step 3

This loop takes the ids_mitogenome values found in Step R3 and, so long as there are at least 1, takes at most 20 mitogenomes, uses Genbank's information on the locus to scrape out the target range, and then appends to the a02_REFDB.

First create a skeleton for the a02_REFDB, and set a counter
    ...
    if (is.na(mito_ids[1]) == FALSE) { 
        ...
        if (class(mito_accessions) != "try-catch"){ 
            ...
            for (n in 1:length(mito_accessions)){ 
              ...
              # update the database
              a02_REFDB <- rbind(a02_REFDB, new_row) 
...
write.csv(a02_REFDB_parsed, file.path(path_outputs, "a02_REFDB.csv"), row.names=FALSE)
```

## Step R5: Scrape Accession Sequences

To scrape the accession sequences from our species list and add them to the a02_REFDB object, prep the loop with a counter and then run the loop

### NTS: restart
Hitting a differing number of rows error at 4573. Save the interim a02_REFDB and restart session
...
## *Step G4: Scrape Mitogenomes for Missing Order Representatives
...
For this loop, we repeat the accession scrape as we did for the species list, but this time just for the species representatives of the missing orders. The sequence results will still append to a02_REFDB to join the rest of the sequences. If you want them to be in their own dataframe, just copy the structure of the empty a02_REFDB and name it something else
...
specieslist <- merge(specieslist, counts_species, by.x= "search_name", by.y="species", all.x=TRUE, all.y=TRUE) # update the summary database with reference sequence counts
...
write.csv(a03_UNIQUEDB, file.path(path_outputs, "a03_UniqueRefDB.csv"), row.names=FALSE)
```
...
```{r format_dada2}
a04_UNIQUEDBdada <- merge(x=a03_UNIQUEDB, y=specieslist, by.x="species", by.y="search_name", all.x=TRUE, all.y=FALSE) 
...
write.table(REFDB_dada, file.path(path_outputs, "a04_REFDBdada.fasta"), sep="\n", col.names=FALSE, row.names=FALSE, quote=FALSE)
```
